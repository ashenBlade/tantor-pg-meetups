...
Можете заметить, что в настоящей реализации есть несколько моментов, которые мы
упустили:

- Проверка изменчивости функции - `VOLATILE` функции не стоит просто так отбрасывать
- Использование `COLLATE` при сравнении
