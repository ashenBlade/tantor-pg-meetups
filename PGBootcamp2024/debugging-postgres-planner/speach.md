...
один из главных компонентов СУБД - он создает из запроса план его выполнения.
...
Вычисляемые узлы - это узлы, которые представляют какие-то выражения:  Var -
    столбец таблицы, Const - константы, OpExpr - операторы (сложения или вычитания).
...
Узлы плана планировщика - это не узлы плана выполнения, они служат планировщику,
    для принятия им решений и содержат столько данных сколько необходимо.
    Например, селективность пути
...
`PlannerInfo` - это информация планировщика о запросе. Содержит в себе все
    необходимое для создания плана выполнения этого запроса. То есть, если есть
    подзапросы, то эта структура будет создана и для них.
...
называется Range Table Entry, сокращенно RTE. Каждый тип RTE имеет свое значение
RTEKind. например, `RTE_RELATION` - это таблица.

Заметьте, что таблицы, функции и подзапросы имеют 2 общих черты: у них есть
название (чтобы уметь к ним обращаться) и список возвращаемых атрибутов. Такие
отношения называются базовыми (в оригинале base relation или base rel). Обычно
планировщик работает не с таблицами, функциями или подзапросами, а с base rel.

На слайде справа вы можете заметить отображение RTEKind в RelOptKind - тип
нашего `RelOptInfo`. Видим, что таблицы, функции и подзапросы - это все base rel.

Для JOIN'ов тоже существует свой тип - RELOPT_JOINREL.

Кроме этих значений, есть и другие. Они нужны, например, для реализации UNION ALL
или наследования таблиц. О них говорить не будем.
