...
Планируется небольшой лайв-кодинг. Кто хочет, клонируйте этот репозиторий и в
папке `repo` будет `README` с шагами настройки.
...
`PlannerInfo` - это информация планировщика о подзапросе.
...
значение перечисления, начинающееся на RTE. например, RTE_RELATION - это таблица.
...
не совсем относится к планировщику, но сильно помогает сократить длину итерации
отладки.



А если мы интегрируем их с VS Code с помощью тасок, то можно достичь еще более
быстрой скорости разработки. Например, с помощью такой таски можно запускать
сборку нажатием шортката Ctrl + Shift + B.

В репозитории я выложу эти скрипты и файлы конфигурации для VS Code. Конечно,
